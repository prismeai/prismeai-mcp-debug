...
| **O — OAuth-only** | `ensureAuthentication`, `oauthCallback`, `refreshOAuthToken`, `initiateOAuth`, `checkAuthStatus`, `disconnectOAuth`, `methodConnect`/`toolConnect`, `methodDisconnect`/`toolDisconnect`, `connect`, `disconnect` | Compared only when the connector has `automations/oauthCallback.yml`. Customized per provider — advisory. |
...
| `R19-camelcase-automations` | MAJOR/NEED_HUMAN | Automation slugs and filenames must be camelCase with no hyphens. Known internal dispatcher renames (`method-restOp` → `methodRestOp`, `tool-connect` → `toolConnect`, etc.) are mechanical if references are updated together. Public App-mode hyphenated slugs are NEED_HUMAN because renaming them changes the tenant-facing `<App>.<automation>` contract. |
